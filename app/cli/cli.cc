...
static int32_t _t_display_status(void* a_data) {
  ...
  if (!g_session->get_info_num_pieces()) {
    if (!s_show_info_progress_heading) {
      ...
      NDBG_OUTPUT(
          "+-------------------------+--------------------------------+\n");
      NDBG_OUTPUT(
          "| %sPeers%s (connected/total) | %sPieces%s (requested/recvd/total) "
          "|\n",
          ANSI_COLOR_FG_YELLOW, ANSI_COLOR_OFF, ANSI_COLOR_FG_BLUE,
          ANSI_COLOR_OFF);
      NDBG_OUTPUT(
          "+-------------------------+--------------------------------+\n");
    ...
    NDBG_OUTPUT(
        "|           %s%6lu%s/%6lu |                    %3lu/%s%3lu%s/%3lu |\r",
        ANSI_COLOR_FG_YELLOW, l_peer_mgr.get_peer_connected_vec().size(),
        ANSI_COLOR_OFF, l_peer_mgr.get_peer_vec().size(),
        l_info_pickr.get_stat_num_pieces_rqstd(), ANSI_COLOR_FG_BLUE,
        l_info_pickr.get_stat_num_pieces_recvd(), ANSI_COLOR_OFF,
        l_info_pickr.get_info_buf_pieces_size());
  } else {
    if (!s_show_progress_heading) {
      if (s_show_info_progress_heading) {
        NDBG_OUTPUT(
            "|           %s%6lu%s/%6lu |                    %3lu/%s%3lu%s/%3lu "
            "|\n",
            ANSI_COLOR_FG_YELLOW, l_peer_mgr.get_peer_connected_vec().size(),
            ANSI_COLOR_OFF, l_peer_mgr.get_peer_vec().size(),
            l_info_pickr.get_stat_num_pieces_rqstd(), ANSI_COLOR_FG_BLUE,
      ...
      NDBG_OUTPUT("%sRequesting Pieces for torrent%s:\n", ANSI_COLOR_FG_CYAN,
                  ANSI_COLOR_OFF);
      NDBG_OUTPUT("  %s%s%s\n", ANSI_COLOR_FG_WHITE,
                  l_info_pickr.get_info_name().c_str(), ANSI_COLOR_OFF);
      NDBG_OUTPUT(
          "+-------------------------+--------------------------+--------------"
          "--------+\n");
      NDBG_OUTPUT(
          "| %sPeers%s (connected/total) | %sBlocks%s (requested/recvd) | "
          "%sPieces%s (recvd/total) |\n",
          ANSI_COLOR_FG_YELLOW, ANSI_COLOR_OFF, ANSI_COLOR_FG_MAGENTA,
          ANSI_COLOR_OFF, ANSI_COLOR_FG_GREEN, ANSI_COLOR_OFF);
      NDBG_OUTPUT(
          "+-------------------------+--------------------------+--------------"
          "--------+\n");
    ...
    NDBG_OUTPUT(
        "|           %s%6lu%s/%6lu |        %8lu/%s%8lu%s |    %s%8lu%s/%8lu "
        "|\r",
        ANSI_COLOR_FG_YELLOW, l_peer_mgr.get_peer_connected_vec().size(),
        ANSI_COLOR_OFF, l_peer_mgr.get_peer_vec().size(),
        l_pickr.get_stat_num_blocks_rqstd(), ANSI_COLOR_FG_MAGENTA,
        l_pickr.get_stat_num_blocks_recvd(), ANSI_COLOR_OFF, l_recv_chr,
        l_pickr.get_pieces().get_count(), ANSI_COLOR_OFF,
  ...
  l_s = g_session->add_timer((uint32_t)(_T_DISPLAY_STATUS_MS),
                             _t_display_status, (void*)nullptr, &l_timer);
  UNUSED(l_s);
  UNUSED(l_timer);
...
class ntrnt_api_handler : public ns_is2::default_rqst_h {
 public:
  ...
  ntrnt_api_handler(ns_ntrnt::session& a_session)
      : default_rqst_h(), m_route(), m_session(a_session) {}
  ns_is2::h_resp_t do_get(ns_is2::session& a_session, ns_is2::rqst& a_rqst,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
ns_is2::h_resp_t ntrnt_api_handler::do_get(
    ns_is2::session& a_session, ns_is2::rqst& a_rqst,
    const ns_is2::url_pmap_t& a_url_pmap) {
  // -------------------------------------------------
  // get path
  // -------------------------------------------------
  ...
  if (!l_route.empty() && (l_path.find(l_route, 0) != std::string::npos)) {
...
#define _ELIF_API(_str) else if (l_url_path == _str)
  ...
  _ELIF_API("info.json") {
    ...
    if (l_s != NTRNT_STATUS_OK) {
      return send_internal_server_error(a_session,
  ...
  l_api_resp.add_std_headers(ns_is2::HTTP_STATUS_OK, "application/json",
                             l_body.length(), a_rqst.m_supports_keep_alives,
...
static void print_usage(FILE* a_stream, int a_exit_code) {
  ...
  fprintf(a_stream,
          "  -A, --no-accept      disable accepting inbound connections\n");
  fprintf(a_stream,
          "  -P, --peer           connect to single peer (disable tracker "
          "announce)\n");
  fprintf(a_stream,
          "  -T, --trace          tracing (none/error/warn/debug/verbose/all) "
          "(default: none)\n");
  ...
  fprintf(a_stream,
          "  -H, --hprofile       Google heap profiler output file\n");
  fprintf(a_stream, "\n");
#endif
  exit(a_exit_code);
}
...
int main(int argc, char** argv) {
  ...
  while (((unsigned char)l_opt != 255)) {
    l_opt = getopt_long_only(argc, argv, l_short_arg_list.c_str(), l_long_opt,
                             &l_opt_index);
    if (optarg) {
    ...
    switch (l_opt) {
      ...
      // trace
      // -----------------------------------------
#define ELIF_TRACE_STR(_level) \
  else if (strncasecmp(_level, l_arg.c_str(), sizeof(_level)) == 0)
      case 'T': {
        if (0) {}
        ELIF_TRACE_STR("error") {
          ns_ntrnt::trc_log_level_set(ns_ntrnt::TRC_LOG_LEVEL_ERROR);
          l_trace = true;
        }
        ELIF_TRACE_STR("warn") {
          ns_ntrnt::trc_log_level_set(ns_ntrnt::TRC_LOG_LEVEL_WARN);
          l_trace = true;
        }
        ELIF_TRACE_STR("debug") {
          ns_ntrnt::trc_log_level_set(ns_ntrnt::TRC_LOG_LEVEL_DEBUG);
          l_trace = true;
        }
        ELIF_TRACE_STR("verbose") {
          ns_ntrnt::trc_log_level_set(ns_ntrnt::TRC_LOG_LEVEL_VERBOSE);
          l_trace = true;
        }
        ELIF_TRACE_STR("all") {
  ...
  // check for file
  // -------------------------------------------------
  if (l_path.empty() && l_info_hash.empty()) {
    NDBG_ERROR_AT(
        "Error torrent metadata / magnet / info-hash must be specified.\n");
    return STATUS_ERROR;
  }
  // -------------------------------------------------
  // Sigint handler
