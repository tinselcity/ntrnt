...
//! test helpers
//! ----------------------------------------------------------------------------
void nbq_write(ns_ntrnt::nbq& a_nbq, char* a_buf, uint32_t a_write_size,
               uint32_t a_write_per) {
...
//! ----------------------------------------------------------------------------
int32_t verify_contents(ns_ntrnt::nbq& a_nbq, uint64_t a_len,
                        uint16_t a_offset) {
  uint64_t l_read = 0;
  //NDBG_PRINT("a_nbq.read_avail(): %lu\n", a_nbq.read_avail());
  while (a_nbq.read_avail() && (l_read < a_len)) {
