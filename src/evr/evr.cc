...
//! \param:   TODO
//! ----------------------------------------------------------------------------
evr_loop::evr_loop(evr_loop_type_t a_type, uint32_t a_max_events)
    : m_event_pq(),
...
uint32_t evr_loop::dequeue_events(void) {
  ...
  while (!m_event_pq.empty()) {
    ...
    if (!l_event || (l_event->m_magic != EVR_EVENT_MAGIC)) {
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t evr_loop::add_fd(int a_fd, uint32_t a_attr_mask,
                         evr_fd_t* a_evr_fd_event) {
  if (!a_evr_fd_event) {
    return NTRNT_STATUS_ERROR;
  }
  int l_status;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t evr_loop::mod_fd(int a_fd, uint32_t a_attr_mask,
                         evr_fd_t* a_evr_fd_event) {
  if (!a_evr_fd_event) {
    return NTRNT_STATUS_ERROR;
  }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t evr_loop::add_event(uint32_t a_time_ms, evr_event_cb_t a_cb,
                            void* a_data, evr_event_t** ao_event) {
