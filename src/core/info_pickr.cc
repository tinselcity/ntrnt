...
#define _ELIF_FIELD(_str) else if (i_m.first == _str)
    ...
    _ELIF_FIELD("files") {
      ...
      for (auto&& i_m : i_list) {
        ...
        if (l_file.m_len && !l_file.m_path.empty()) {
...
int32_t info_pickr::validate_info(void) {
  ...
  if (l_s != 0) {
    TRC_ERROR("bad hash cmp expected[%s] != checked[%s]",
              m_session.get_info_hash_str().c_str(), l_sha1.get_hash_hex());
...
int32_t info_pickr::peer_request_info(peer& a_peer) {
  ...
  if (!m_info_buf_len) {
    ...
    m_info_buf = (uint8_t*)malloc((sizeof(uint8_t) * m_info_buf_len));
    size_t l_num = (m_info_buf_len / NTRNT_METADATA_PIECE_SIZE) +
                   (m_info_buf_len % NTRNT_METADATA_PIECE_SIZE > 0);
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t info_pickr::recv_info_piece(peer* a_peer, uint32_t a_idx,
                                    const char* a_buf, size_t a_len) {
  ...
  // -------------------------------------------------
  if (l_exp_len != a_len) {
    TRC_ERROR("bad length(%lu) for piece: %u / %lu", a_len, a_idx,
              m_info_buf_pieces.get_size());
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t info_pickr::get_info_piece(peer* a_peer, uint32_t a_idx,
                                   const char** ao_buf, size_t& ao_len) {
  // -------------------------------------------------
  // sanity check
  // -------------------------------------------------
  if (!ao_buf) {
