...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int peer_phe_select_skey_cb(void* a_ctx, void* a_cb_ctx, void* a_buf,
                            size_t a_len) {
  if (!a_ctx || !a_cb_ctx) {
    return NTRNT_STATUS_ERROR;
  }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
peer::peer(peer_from_t a_from, session& a_session, peer_mgr& a_peer_mgr,
...
#define _CASE_PEER_ERR_STR(_err) \
  case peer::ERROR_##_err: {     \
    return #_err;                \
  }
  switch (a_error) {
...
#define _CASE_PEER_FROM_STR(_from) \
  case NTRNT_PEER_FROM_##_from: {  \
    return #_from;                 \
  }
  switch (a_from) {
...
#define _CASE_PEER_STATE_STR(_state) \
  case peer::STATE_##_state: {       \
    return #_state;                  \
  }
  switch (a_state) {
...
#define _CASE_BTP_CMD_STR(_cmd) \
  case peer::BTP_CMD_##_cmd: {  \
    return #_cmd;               \
  }
  switch (a_cmd) {
...
void peer::pr_utp_on_error(int a_error_code) {
  TRC_ERROR("utp[skt: %p] error[%d]: %s", m_utp_conn, a_error_code,
  ...
  switch (a_state) {
    ...
    case UTP_STATE_CONNECT: {
      switch (m_state) {
        ...
        // -----------------------------------------
        default: {
          TRC_ERROR("STATE_CONNECT from peer state[%d] != STATE_UTP_CONNECTING",
                    m_state);
          break;
        }
      }
  ...
  while (true) {
    switch (m_state) {
      ...
      case STATE_PHE_CONNECTING: {
        ...
        if (l_ia_buf && (l_ia_len >= NTRNT_PEER_HANDSHAKE_SIZE)) {
...
int32_t peer::add_timer(uint32_t a_ms) {
  ...
  // -------------------------------------------------
  int32_t l_s;
  l_s = m_session.add_timer(a_ms, _timeout, (void*)this, &m_timer);
  if (l_s != NTRNT_STATUS_OK) {
    TRC_ERROR("performing add_timer");
    return NTRNT_STATUS_ERROR;
  }
...
int32_t peer::btp_parse_handshake_ia(const uint8_t* a_buf, size_t a_len) {
  ...
  // check for self
  // -------------------------------------------------
  if (memcmp(m_btp_peer_id.m_data, m_session.get_peer_id().c_str(),
             sizeof(m_btp_peer_id)) == 0) {
...
int32_t peer::btp_parse_handshake(void) {
  ...
  // check for self
  // -------------------------------------------------
  if (memcmp(m_btp_peer_id.m_data, m_session.get_peer_id().data(),
             sizeof(m_btp_peer_id)) == 0) {
    m_error = ERROR_HANDSHAKE_SELF;
    // -----------------------------------------
    // check for session self address
...
#define _CHECK_LEN(_len)              \
  ...
  if (!m_btp_cmd_len) {
    ...
    if (m_btp_cmd_len > 262144) {
      TRC_ERROR(
          "[HOST: %s] [CLIENT: %s] btp_cmd_len: %d >> than expected -channel "
          "could be corrupted",
          m_host.c_str(), m_ltep_peer_id.c_str(), m_btp_cmd_len);
      return NTRNT_STATUS_ERROR;
    }
  }
  // -------------------------------------------------
  ...
  switch (m_btp_cmd) {
    ...
    default: {
      ...
      TRC_WARN("[HOST: %s] [CLIENT: %s] [BTP] [RECV] unhandled btp cmd: %u\n",
               m_host.c_str(), m_ltep_peer_id.c_str(), m_btp_cmd);
      // discard for now
      m_in_q.discard(m_btp_cmd_len - 1);
      break;
    }
...
int32_t peer::btp_recv_bitfield(void) {
  ...
  UNUSED(l_rd);
  int32_t l_s;
  l_s = m_btp_pieces_have.import_raw((const uint8_t*)l_btfield, l_len,
...
int32_t peer::ltep_send_pex(void) {
  ...
  for (auto&& i_p : l_pv) {
    ...
    if (l_sas.ss_family == AF_INET) {
      ...
      // add flags
      l_peers4_f.insert(l_peers4_f.end(), &l_flags,
                        (&l_flags) + sizeof(l_flags));
    }
    // -----------------------------------------
    // add ipv4 peer
    // -----------------------------------------
      ...
      // add flags
      l_peers6_f.insert(l_peers6_f.end(), &l_flags,
                        (&l_flags) + sizeof(l_flags));
    }
  }
  // -------------------------------------------------
...
int32_t peer::ltep_send_metadata(uint32_t a_idx) {
  ...
  l_s = m_session.get_info_pickr().get_info_piece(this, a_idx, &l_metadata_buf,
                                                  l_metadata_buf_len);
  if (l_s != NTRNT_STATUS_OK) {
...
int32_t peer::ltep_read_cmd(void) {
  ...
  // -------------------------------------------------
    ...
    TRC_WARN("[HOST: %s] [CLIENT: %s] unhandled ltep cmd: %u\n", m_host.c_str(),
             m_ltep_peer_id.c_str(), m_btp_cmd);
    // discard for now
...
int32_t peer::ltep_recv_handshake(size_t a_len) {
  ...
  size_t l_rem = 0;
  size_t l_len;
  l_len = a_len < NTRNT_PEER_LTEP_HANDSHAKE_SIZE
              ? a_len
              : NTRNT_PEER_LTEP_HANDSHAKE_SIZE;
  ...
  if ((size_t)l_read != l_len) {
    TRC_ERROR("read < required [read: %d] [requested: %lu]", (int)l_read,
              l_len);
    return NTRNT_STATUS_ERROR;
  }
...
#define _ELIF_UT_FIELD(_str) else if (i_m.first == _str)
  ...
  // add ipv4
  // -------------------------------------------------
  if (l_added_buf && l_added_buf_len) {
    int32_t l_s;
    l_s = m_session.add_peer_raw(AF_INET, (const uint8_t*)l_added_buf,
                                 l_added_buf_len,
                                 (const uint8_t*)l_added_flags_buf,
                                 l_added_flags_buf_len, NTRNT_PEER_FROM_PEX);
    if (l_s != NTRNT_STATUS_OK) {
  ...
  // add ipv4
  // -------------------------------------------------
  if (l_added6_buf && l_added6_buf_len) {
    int32_t l_s;
    l_s = m_session.add_peer_raw(AF_INET, (const uint8_t*)l_added6_buf,
                                 l_added6_buf_len,
                                 (const uint8_t*)l_added6_flags_buf,
                                 l_added6_flags_buf_len, NTRNT_PEER_FROM_PEX);
    if (l_s != NTRNT_STATUS_OK) {
      TRC_ERROR("performing add_peer_raw(AF_INET)");
      if (l_buf) {
        free(l_buf);
...
int32_t peer::ltep_recv_metadata(size_t a_len) {
  ...
  if (l_type == LTEP_METADATA_CMD_DATA) {
    ...
    l_s = m_session.get_info_pickr().recv_info_piece(this, l_idx, l_cur_ptr,
                                                     l_cur_len - l_cur_off);
    if (l_s != NTRNT_STATUS_OK) {
