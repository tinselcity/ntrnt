...
typedef struct _sas_comp {
  inline bool operator()(const sockaddr_storage& lhs,
                         const sockaddr_storage& rhs) const {
    return (memcmp(&lhs, &rhs, sizeof(lhs)) == 0);
  }
} sas_comp_t;
typedef std::unordered_map<sockaddr_storage, peer*, sas_hash_t, sas_comp_t>
    peer_map_t;
...
class peer_mgr {
 public:
  ...
  int32_t pm_utp_sendto(const uint8_t* a_buf, size_t a_len,
                        const struct sockaddr* a_sa, socklen_t a_socklen);
  // -------------------------------------------------
  // apis
  // -------------------------------------------------
