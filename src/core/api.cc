...
int32_t session::api_get_trackers(std::string& ao_body) {
  ...
  for (auto&& i_t : m_tracker_list) {
    ...
    l_writer.Uint64(l_t.m_stat_last_announce_num_peers +
                    l_t.m_stat_last_announce_num_peers6);
    // -----------------------------------------
    // scrape
    // -----------------------------------------
...
int32_t peer_mgr::get_peers_api(std::string& ao_body) {
  ...
  for (auto&& i_p : m_peer_vec) {
    ...
    l_writer.Key("from");
    switch (l_p.get_from()) {
      case NTRNT_PEER_FROM_NONE: {
        l_writer.String("NONE");
        break;
      }
      case NTRNT_PEER_FROM_SELF: {
        l_writer.String("SELF");
        break;
      }
      case NTRNT_PEER_FROM_TRACKER: {
        l_writer.String("TRACKER");
        break;
      }
      case NTRNT_PEER_FROM_DHT: {
        l_writer.String("DHT");
        break;
      }
      case NTRNT_PEER_FROM_INBOUND: {
        l_writer.String("INBOUND");
        break;
      }
      case NTRNT_PEER_FROM_PEX: {
        l_writer.String("PEX");
        break;
      }
      default: {
    ...
    switch (l_p.get_state()) {
      case peer::STATE_NONE: {
        l_writer.String("NONE");
        break;
      }
      case peer::STATE_UTP_CONNECTING: {
        l_writer.String("UTP_CONNECTING");
        break;
      }
      case peer::STATE_PHE_SETUP: {
        l_writer.String("PHE_SETUP");
        break;
      }
      case peer::STATE_PHE_CONNECTING: {
        l_writer.String("PHE_CONNECTING");
        break;
      }
      case peer::STATE_HANDSHAKING: {
        l_writer.String("HANDSHAKING");
        break;
      }
      case peer::STATE_CONNECTED: {
        l_writer.String("CONNECTED");
        break;
      }
      default: {
        l_writer.String("NONE");
    ...
    switch (l_p.get_error()) {
      ...
      case peer::ERROR_TIMEOUT: {
        l_writer.String("TIMEOUT");
        break;
      }
      case peer::ERROR_EOF: {
        l_writer.String("EOF");
      ...
      case peer::ERROR_EXPIRED_BR: {
        l_writer.String("EXPIRED_BR");
        break;
      }
      case peer::ERROR_IDLE_TIMEOUT: {
        l_writer.String("IDLE_TIMEOUT");
        break;
      }
      case peer::ERROR_CONNECT: {
        l_writer.String("CONNECT");
        break;
      }
      case peer::ERROR_UTP_EOF: {
        l_writer.String("UTP_EOF");
        break;
      }
      case peer::ERROR_UTP_CB_DONE: {
        l_writer.String("UTP_CB_DONE");
        break;
      }
      case peer::ERROR_UTP_CB_ERROR: {
        l_writer.String("UTP_CB_ERROR");
        break;
      }
      default: {
        l_writer.String("NONE");
