...
typedef struct _blocks {
  size_t m_len;
  btfield m_btfield;
  _blocks(size_t a_len) : m_len(a_len), m_btfield() {
...
typedef struct _block_rqst {
  ...
  uint32_t m_len;
  uint64_t m_time_ms;
  _block_rqst() : m_ctx(nullptr), m_idx(0), m_off(0), m_len(0), m_time_ms(0) {}
} block_rqst_t;
...
class pickr {
 public:
  ...
  int32_t get_piece(peer* a_peer, uint32_t a_idx, uint32_t a_off,
                    uint32_t a_len, nbq* a_q);
  int32_t recv_piece(peer* a_peer, nbq& a_q, uint32_t a_idx, uint32_t a_off,
                     uint32_t l_len);
  // -------------------------------------------------
  // housekeeping
  // -------------------------------------------------
 ...
 private:
  ...
  int32_t get_block_rqsts(block_rqst_vec_t& ao_vec, peer& a_peer,
