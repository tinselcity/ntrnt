...
static uint64 _pm_utp_log(utp_callback_arguments* a_args) {
  ...
  // -------------------------------------------------
  peer_mgr* l_pm =
      static_cast<peer_mgr*>(utp_context_get_userdata(a_args->context));
  if (!l_pm) {
...
int32_t peer_mgr::init(void) {
  ...
  utp_set_callback(m_utp_ctx, UTP_GET_READ_BUFFER_SIZE,
                   _pm_utp_get_read_buffer_size);
  utp_set_callback(m_utp_ctx, UTP_ON_ERROR, _pm_utp_on_error);
  utp_set_callback(m_utp_ctx, UTP_ON_OVERHEAD_STATISTICS,
...
//#ifdef UTP_DEBUG_LOGGING
  ...
  int32_t l_s;
  l_s = utp_context_set_option(m_utp_ctx, UTP_RCVBUF,
  ...
  l_s = m_session.add_timer((uint32_t)(NTRNT_SESSION_T_CHECK_TIMEOUTS_MS),
                            _pm_utp_check_timeouts, (void*)this, &l_timer);
  UNUSED(l_s);
  UNUSED(l_timer);
  // -------------------------------------------------
  // done
...
void peer_mgr::display(void) {
  ...
  for (auto&& i_p : m_peer_map) {
    ...
    if (l_p.get_btp_am_interested() && l_p.get_btp_peer_choking()) {
      ++l_choked;
    }
  }
  NDBG_OUTPUT("PEERS");
...
void peer_mgr::display_peers(void) {
  ...
  for (auto&& i_p : m_peer_map) {
    ...
    NDBG_OUTPUT("HOST: %s STATE: %d FROM: %d\n", i_p.second->get_host().c_str(),
                i_p.second->get_state(), i_p.second->get_from());
  }
}
//! ----------------------------------------------------------------------------
//! \details: TODO
...
int32_t peer_mgr::connect_peers(void) {
  ...
  for (auto&& i_p : m_peer_connected_vec) {
    ...
    if (l_exp > (uint64_t)l_p.get_ltep_reqq()) {
      TRC_WARN(
          "[HOST: %s] [CLIENT: %s] [PEER: %s] seems down -removing from swarm "
          "[expired count: %u]",
          l_p.m_host.c_str(), l_p.m_btp_peer_str.c_str(),
          l_p.m_ltep_peer_id.c_str(), (unsigned int)l_exp);
    ...
    // -----------------------------------------
    if (l_now_s >
        (l_p.m_stat_last_recvd_time_s + NTRNT_SESSION_PEER_MAX_IDLE_S)) {
      TRC_WARN(
          "[HOST: %s] [CLIENT: %s] [PEER: %s] seems down -removing from swarm "
          "[last msg recvd: %u s ago]",
          l_p.m_host.c_str(), l_p.m_btp_peer_str.c_str(),
  ...
  for (auto&& i_p : m_peer_vec) {
    ...
    // -----------------------------------------
    ++l_states[l_ps];
    l_p.m_stat_bytes_recv_per_s =
        (uint64_t)((double)(l_p.m_stat_bytes_recv -
                            l_p.m_stat_bytes_recv_last) *
                   (1000.0 / (double)(NTRNT_SESSION_T_CONNECT_PEERS_MS)));
    l_p.m_stat_bytes_recv_last = l_p.m_stat_bytes_recv;
    l_p.m_stat_bytes_sent_per_s =
        (uint64_t)((double)(l_p.m_stat_bytes_sent -
                            l_p.m_stat_bytes_sent_last) *
                   (1000.0 / (double)(NTRNT_SESSION_T_CONNECT_PEERS_MS)));
  ...
  _peer_vector_t l_pv;
  pthread_mutex_lock(&m_mutex);
  for (size_t i_pc = 0; (i_pc < m_peer_vec.size()) && (l_pv.size() <= l_num);
       ++i_pc) {
...
int32_t peer_mgr::set_geoip(peer& a_peer, const sockaddr_storage& a_sas) {
  ...
  l_s =
      l_geoip2_mmdb->get_geoip_data(&l_cn, l_cn_len, &l_city, l_city_len, l_lat,
                                    l_lon, l_ip_str.c_str(), l_ip_str.length());
...
int32_t peer_mgr::validate_address(const sockaddr_storage& a_sas) {
  ...
  // -------------------------------------------------
    ...
    if ((l_addr[0] == 0xFF) ||
        ((memcmp(l_addr, s_zeros, 15) == 0) &&
         ((l_addr[15] == 0x00) || (l_addr[15] == 0x01)))) {
      TRC_ERROR("address[%s] ipv6 is default or unspecified",
                sas_to_str(a_sas).c_str());
      return NTRNT_STATUS_ERROR;
    }
  } else {
    TRC_ERROR("address[%s] unrecognized address family: %d",
              sas_to_str(a_sas).c_str(), a_sas.ss_family);
    return NTRNT_STATUS_ERROR;
  }
  // -------------------------------------------------
  // check for zero port
  // -------------------------------------------------
  if (l_port == 0) {
    TRC_ERROR("address[%s] bad address port is zero",
              sas_to_str(a_sas).c_str());
    return NTRNT_STATUS_ERROR;
  }
  // -------------------------------------------------
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t peer_mgr::add_peer(const sockaddr_storage& a_sas, peer_from_t a_from) {
  int32_t l_s;
  // -------------------------------------------------
  // validate address
  // -------------------------------------------------
...

{
  ...
  if (l_sa->sa_family == AF_INET) {
    ...
    TRC_ERROR("unrecognized address family: %u len: %u", l_sa->sa_family,
              l_sa_len);
    utp_close(a_utp_conn);
    return NTRNT_STATUS_ERROR;
  }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t peer_mgr::pm_utp_sendto(const uint8_t* a_buf, size_t a_len,
...
int32_t peer_mgr::dequeue_out_v4(void) {
  ...
  for (auto&& i_p : m_peer_active_vec_v4) {
    ...
    if (!l_utp_conn) {
      TRC_ERROR("[PEER: %s] [STATE: %d] utp_conn == null",
                l_p.get_host().c_str(), l_p.get_state());
      return NTRNT_STATUS_OK;
    }
    // -----------------------------------------
    // write until EAGAIN
