...
int32_t init_tracker_w_url(tracker** ao_tracker, session& a_session,
  ...
  if ((!a_url) || (a_url_len <= 0) || (!ao_tracker)) {
...
void http_escape(std::string& ao_out, std::string a_in, bool a_escp_rsvd) {
  ...
  static const std::string s_unescpd_chars =
      "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-_.~";
  for (auto& ch : a_in) {
    if ((s_unescpd_chars.find(ch) != s_unescpd_chars.npos) ||
        (!a_escp_rsvd && (s_rsvd_chars.find(ch) != s_rsvd_chars.npos))) {
      ao_out += ch;
    } else {
      char buf[16];
...
void encode_digest(char* a_out, const uint8_t* a_digest, size_t a_digest_len) {
  for (size_t i_c = 0; i_c < a_digest_len; ++i_c) {
    ...
    if (('0' <= l_c && l_c <= '9') || ('A' <= l_c && l_c <= 'Z') ||
        ('a' <= l_c && l_c <= 'z') || l_c == '.' || l_c == '-' || l_c == '_' ||
        l_c == '~') {
      *a_out++ = (char)l_c;
    } else {
