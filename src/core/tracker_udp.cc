...
class tracker_udp_rqst {
 public:
  ...
  // -------------------------------------------------
  // type
  typedef enum { TYPE_NONE = 0, TYPE_ANNOUNCE, TYPE_SCRAPE } type_t;
  // state
  typedef enum {
    STATE_NONE = 0,
    STATE_QUEUED,
  ...
  switch (m_state) {
    ...
    case tracker_udp_rqst::STATE_CONNECTED: {
      ...
      if (m_type == tracker_udp_rqst::TYPE_ANNOUNCE) {
        ...
        m_tracker.m_next_announce_s =
            get_time_s() + NTRNT_SESSION_TRACKER_ANNOUNCE_S;
      } else if (m_type == tracker_udp_rqst::TYPE_SCRAPE) {
        l_s = recv_scrape(a_msg, a_msg_len);
        if (l_s != NTRNT_STATUS_OK) {
          return NTRNT_STATUS_ERROR;
        }
        m_tracker.m_next_scrape_s =
            get_time_s() + NTRNT_SESSION_TRACKER_ANNOUNCE_S;
      }
...
int32_t tracker_udp_rqst::send_connect(void) {
  ...
  // -------------------------------------------------
  errno = 0;
  l_s = sendto(m_fd, l_msg, sizeof(l_msg), 0,
               (struct sockaddr*)(&m_host_info.m_sa), m_host_info.m_sa_len);
  if (l_s < 0) {
    // -----------------------------------------
    // EAGAIN
    // -----------------------------------------
...
int32_t tracker_udp_rqst::send_announce(void) {
  ...
  // -------------------------------------------------
  errno = 0;
  l_s = sendto(m_fd, l_ann, sizeof(l_ann), 0,
               (struct sockaddr*)(&m_host_info.m_sa), m_host_info.m_sa_len);
  if (l_s < 0) {
    // -----------------------------------------
    // EAGAIN
...
int32_t tracker_udp_rqst::recv_announce(uint8_t* a_msg, uint32_t a_msg_len) {
  ...
  if (m_host_info.m_sock_family == AF_INET) {
    int32_t l_s;
    l_s = m_tracker.m_session.add_peer_raw(AF_INET, a_msg + l_off, l_p_size,
                                           nullptr, 0, NTRNT_PEER_FROM_TRACKER);
    UNUSED(l_s);
  }
  // -------------------------------------------------
  ...
  // -------------------------------------------------
    ...
    int32_t l_s;
    l_s = m_tracker.m_session.add_peer_raw(AF_INET6, a_msg + l_off, l_p_size,
                                           nullptr, 0, NTRNT_PEER_FROM_TRACKER);
    UNUSED(l_s);
  }
  return NTRNT_STATUS_OK;
...
tracker_udp::tracker_udp(session& a_session)
    : tracker(a_session), m_gc_udp_rqst_list() {}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
//! \param:   TODO
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t tracker_udp::handle_resp(tid_tracker_udp_map_t& a_map, uint8_t* a_msg,
                                 uint32_t a_msg_len) {
  off_t l_off = 0;
  // -------------------------------------------------
  // read action/tx id
