...
// ---------------------------------------------------------
static const uint8_t _g_dhe_generator[] = {2};
...
// ---------------------------------------------------------
static const uint8_t _g_dhe_vc[] = {0x00, 0x00, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x00};
namespace ns_ntrnt {
//! ----------------------------------------------------------------------------
//! arc4
 ...
 public:
  ...
  void discard(size_t a_len) {
    ...
    for (size_t i_c = 0; i_c < a_len; ++i_c) {
      RC4(&m_ctx, sizeof(l_c), (const unsigned char*)&l_c,
          (unsigned char*)&l_c);
    }
#else
    for (size_t i = 0; i < a_len; ++i) {
...
int32_t phe::init(void) {
  ...
  // -------------------------------------------------
  BIGNUM* const l_g =
      BN_bin2bn(_g_dhe_generator, sizeof(_g_dhe_generator), nullptr);
...
int32_t phe::recv_ya(nbq& a_in_q) {
  ...
  // -------------------------------------------------
  BIGNUM* const l_ya =
      BN_bin2bn((const unsigned char*)l_buf, PHE_PUBLIC_KEY_SIZE, nullptr);
  // -------------------------------------------------
  // get secret
  // S = (Yb ^ Xa) mod P
  // -------------------------------------------------
...
int32_t phe::send_ab(nbq& a_out_q) {
  ...
  // -------------------------------------------------
  sha1 l_req1_sha;
  l_req1_sha.update((const uint8_t*)_AB_MSG_STR_REQ1,
                    sizeof(_AB_MSG_STR_REQ1) - 1);
  ...
  // -------------------------------------------------
  sha1 l_req2_sha;
  l_req2_sha.update((const uint8_t*)_AB_MSG_STR_REQ2,
                    sizeof(_AB_MSG_STR_REQ2) - 1);
  l_req2_sha.update(m_skey, m_skey_len);
  l_req2_sha.finish();
  const uint8_t* l_req2_sha_dat = l_req2_sha.get_hash();
  sha1 l_req3_sha;
  l_req3_sha.update((const uint8_t*)_AB_MSG_STR_REQ3,
                    sizeof(_AB_MSG_STR_REQ3) - 1);
  ...
  // -------------------------------------------------
  sha1 l_key_sha;
  l_key_sha.update((const uint8_t*)_PHE_ENCRYPT_KEYA,
                   sizeof(_PHE_ENCRYPT_KEYA) - 1);
  ...
  // ENCRYPT(VC, crypto_provide, len(PadC), PadC, len(IA))
  // -------------------------------------------------
  uint32_t l_crypto_provide =
      PHE_CRYPTO_PROVIDE_RC4 | PHE_CRYPTO_PROVIDE_PLAINTEXT;
  ...
  m_encrypt->process(l_pc3 + l_off, &l_crypto_provide,
                     sizeof(l_crypto_provide));
...
int32_t phe::send_ba(nbq& a_out_q) {
  ...
  // -------------------------------------------------
  sha1 l_key_sha;
  l_key_sha.update((const uint8_t*)_PHE_ENCRYPT_KEYB,
                   sizeof(_PHE_ENCRYPT_KEYB) - 1);
  l_key_sha.update(m_secret, sizeof(m_secret));
  l_key_sha.update(m_skey, m_skey_len);
  l_key_sha.finish();
  // -------------------------------------------------
...
int32_t phe::recv_ab_needle(nbq& a_in_q) {
  ...
  while (true) {
    ...
    if (a_in_q.starts_with((const char*)l_req1_sha.get_hash(),
                           NTRNT_SHA1_SIZE)) {
...
int32_t phe::recv_ab(nbq& a_in_q) {
  ...
  // 20 + 8 + 4 + 2
  // -------------------------------------------------
  size_t l_require_len = NTRNT_SHA1_SIZE + sizeof(_g_dhe_vc) +
