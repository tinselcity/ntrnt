...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int _dhsco_sendto(int a_sockfd, const void* a_buf, int a_len,
                         int a_flags, const struct sockaddr* a_to,
                         int a_to_len) {
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static void _dhsco_hash(void* a_hash_return, int a_hash_size, const void* a_v1,
                        int a_len1, const void* a_v2, int a_len2,
                        const void* a_v3, int a_len3) {
  // -------------------------------------------------
  // sha1 impl
  // -------------------------------------------------
...
dht_mgr::dht_mgr(session& a_session)
    : m_session(a_session), m_dhsco(nullptr), m_id() {}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
//! \param:   TODO
...
int32_t dht_mgr::init(void) {
  ...
  // setup dht
  // -------------------------------------------------
  m_dhsco = new dhsco(m_session.get_udp_fd(), m_session.get_udp6_fd(),
                      m_id.m_data, nullptr);
  // -------------------------------------------------
  ...
  // kick off bootstrap
  // -------------------------------------------------
  l_s = m_session.add_timer(_DHT_BOOTSTRAP_DEQUEUE_TIME_MS, _bootstrap,
                            (void*)this, &l_timer);
  UNUSED(l_s);
  UNUSED(l_timer);
  // -------------------------------------------------
  // kick off announce
  // -------------------------------------------------
  l_s = m_session.add_timer(_DHT_ANNOUNCE_TIME_MS, _announce, (void*)this,
                            &l_timer);
  UNUSED(l_s);
  UNUSED(l_timer);
  // -------------------------------------------------
  // kick off periodic
  // -------------------------------------------------
  l_s = m_session.add_timer(_DHT_PERIODIC_FIRST_TIME_MS, _periodic, (void*)this,
                            &l_timer);
  UNUSED(l_s);
  UNUSED(l_timer);
  return NTRNT_STATUS_OK;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t dht_mgr::recv_msg(struct sockaddr_storage& a_ss, socklen_t& a_ss_len,
  ...
  time_t l_to_sleep;
  int32_t l_s;
  l_s = m_dhsco->periodic((const void*)a_msg, (size_t)a_msg_len,
                          (const struct sockaddr*)&a_ss, a_ss_len, &l_to_sleep,
                          dht_cb, this);
  if (l_s != NTRNT_STATUS_OK) {
    TRC_ERROR("performing dht_periodic.");
    return NTRNT_STATUS_ERROR;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
void dht_mgr::dht_cb(void* a_ctx, dht_event_t a_event,
                     uint8_t const* a_info_hash, void const* a_data,
                     size_t a_data_len) {
  if (!a_ctx) {
  ...
  switch (a_event) {
    ...
    case DHT_EVENT_VALUES: {
      int32_t l_s;
      l_s = l_ses.add_peer_raw(AF_INET, (uint8_t*)a_data, a_data_len, nullptr,
                               0, NTRNT_PEER_FROM_DHT);
      UNUSED(l_s);
      break;
    }
    // -------------------------------------------------
    ...
    case DHT_EVENT_VALUES6: {
      int32_t l_s;
      l_s = l_ses.add_peer_raw(AF_INET6, (uint8_t*)a_data, a_data_len, nullptr,
                               0, NTRNT_PEER_FROM_DHT);
      UNUSED(l_s);
      break;
    }
...
int32_t dht_mgr::announce(void) {
  ...
  // ipv4
  // -------------------------------------------------
  l_s = m_dhsco->search(m_session.get_info_hash(), m_session.get_udp_fd(),
                        AF_INET, dht_cb, this);
  if (l_s != NTRNT_STATUS_OK) {
  ...
  // ipv6
  // -------------------------------------------------
  l_s = m_dhsco->search(m_session.get_info_hash(), m_session.get_udp6_fd(),
                        AF_INET6, dht_cb, this);
  if (l_s != NTRNT_STATUS_OK) {
    //TRC_ERROR("performing dhsco search");
    // soft fail
  }
...
int32_t dht_mgr::periodic(void) {
  ...
  // -------------------------------------------------
  time_t l_to_sleep;
  l_s = m_dhsco->periodic((const void*)nullptr, (size_t)0,
                          (const struct sockaddr*)nullptr, 0, &l_to_sleep,
                          dht_cb, this);
  if (l_s != NTRNT_STATUS_OK) {
    TRC_ERROR("performing dht_periodic.");
    return NTRNT_STATUS_ERROR;
  ...
  // -------------------------------------------------
  void* l_timer = NULL;
  l_s = m_session.add_timer((uint32_t)(l_to_sleep * 1000), _periodic,
                            (void*)this, &l_timer);
  UNUSED(l_s);
  // TODO Check status...
  return NTRNT_STATUS_OK;
