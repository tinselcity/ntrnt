...
class session {
 public:
  ...
  int32_t add_peer_raw(int a_family, const uint8_t* a_buf, size_t a_len,
                       const uint8_t* a_flags_buf, size_t a_flags_len,
                       peer_from_t a_from);
  // -------------------------------------------------
  // udp helpers
  // -------------------------------------------------
  int32_t udp_mux(struct sockaddr_storage& a_ss, socklen_t& a_ss_len,
                  uint8_t* a_msg, uint32_t a_msg_len);
  // -------------------------------------------------
  // timer helper
  // -------------------------------------------------
  int32_t add_timer(uint32_t a_time_ms, evr_event_cb_t a_cb, void* a_data,
                    void** ao_event);
  ...
  uint32_t get_info_num_pieces(void) {
    return (uint32_t)m_info_pickr.get_info_pieces_size();
  }
  ...
  void set_ext_address_v4(const std::string& a_str) {
    m_ext_address_v4 = a_str;
  }
  void set_ext_address_v6(const std::string& a_str) {
    m_ext_address_v6 = a_str;
  }
  // -------------------------------------------------
  // geoip2 support
  // -------------------------------------------------
