...
int32_t nbq_write_status(nbq& ao_q, http_status_t a_status) {
  http_resp_strs::code_resp_map_t::const_iterator i_r =
      http_resp_strs::S_CODE_RESP_MAP.find(a_status);
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t nbq_write_header(nbq& ao_q, const char* a_key_buf, uint32_t a_key_len,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t nbq_write_header(nbq& ao_q, const char* a_key_buf,
                         const char* a_val_buf) {
  nbq_write_header(ao_q, a_key_buf, strlen(a_key_buf), a_val_buf,
                   strlen(a_val_buf));
  return 0;
}
//! ----------------------------------------------------------------------------
...
class tracker_tcp_rqst {
 public:
  ...
  static int32_t teardown(tracker_tcp_rqst* a_rqst, nconn& a_nconn,
...
static int32_t run_state_machine(void* a_data, evr_mode_t a_conn_mode) {
  ...
  switch (a_conn_mode) {
    ...
    case EVR_MODE_ERROR: {
      ...
      int32_t l_s;
      l_s =
          tracker_tcp_rqst::teardown(l_rqst, l_nconn, HTTP_STATUS_BAD_GATEWAY);
      // TODO -check status...
      UNUSED(l_s);
      return NTRNT_STATUS_DONE;
    }
    ...
    case EVR_MODE_TIMEOUT: {
      ...
      if (((uint32_t)(l_ct_ms - l_rqst->m_last_active_ms)) >=
          l_rqst->m_timeout_ms) {
        ...
        TRC_ERROR("connection error: label: %s\n", l_nconn.get_label().c_str());
        int32_t l_s;
        l_s = tracker_tcp_rqst::teardown(l_rqst, l_nconn,
    ...
    case nconn::NC_STATE_CONNECTED: {
      switch (a_conn_mode) {
        ...
        case EVR_MODE_READ: {
          ...
          if (l_s != nconn::NC_STATUS_OK) {
            switch (l_s) {
              ...
              case nconn::NC_STATUS_EOF: {
                ...
                int32_t l_s;
                l_s =
                    tracker_tcp_rqst::teardown(l_rqst, l_nconn, HTTP_STATUS_OK);
                // TODO -check status...
                UNUSED(l_s);
                return NTRNT_STATUS_DONE;
              }
          ...
          // parse
          // ---------------------------------
          if ((l_read > 0) && l_rqst && l_rqst->m_resp &&
            ...
            l_parse_status = http_parser_execute(
                l_hmsg->m_http_parser, l_hmsg->m_http_parser_settings,
                reinterpret_cast<const char*>(l_buf), l_read);
            if (l_parse_status < (size_t)l_read) {
              TRC_ERROR(
                  "Parse error.  Reason: %s: %s\n",
                  http_errno_name(
                      (enum http_errno)l_hmsg->m_http_parser->http_errno),
          ...
          // handle completion
          // ---------------------------------
          if (l_rqst && l_rqst->m_resp && l_rqst->m_resp->m_complete) {
            // -------------------------
            // Cancel timer
            // -------------------------
            ...
            if (l_rqst->m_resp) {
              if (l_rqst->m_scrape) {
                int32_t l_hs;
                l_hs =
                    l_rqst->m_tracker.handle_scrape_response(*(l_rqst->m_resp));
                UNUSED(l_hs);
                // TODO check resp
              } else {
                int32_t l_hs;
                l_hs = l_rqst->m_tracker.handle_announce_response(
                    *(l_rqst->m_resp));
                UNUSED(l_hs);
                // TODO check resp
              }
            ...
            if (!l_nconn_can_reuse || !l_keepalive || !l_hmsg_keep_alive) {
              TRC_VERBOSE(
                  "marking conn EOF: l_nconn_can_reuse: %d, l_keepalive: %d, "
                  "l_hmsg_keep_alive: %d\n",
                  l_nconn_can_reuse, l_keepalive, l_hmsg_keep_alive);
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int tracker_tcp_rqst::set_query(const std::string& a_key,
...
int32_t tracker_tcp_rqst::serialize(nbq& ao_q) {
  ...
  int32_t l_len = 0;
  char l_buf[2048];
  l_len = snprintf(l_buf, sizeof(l_buf), "%s %s HTTP/1.1", m_verb.c_str(),
                   l_path.c_str());
...
int32_t tracker_tcp_rqst::start(session& a_session) {
  ...
  l_s = l_resolver.lookup_tryfast(m_host, m_port, l_host_info);
  ...
  if (m_scheme == SCHEME_TCP) {
    m_nconn = new nconn_tcp();
  } else if (m_scheme == SCHEME_TLS) {
    m_nconn = new nconn_tls();
  }
  if (!m_nconn) {
  ...
  if (m_nconn->get_scheme() == SCHEME_TLS) {
    ...
    _SET_NCONN_OPT((*m_nconn), nconn_tls::OPT_TLS_HOSTNAME, m_host.c_str(),
                   m_host.length());
  }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t tracker_tcp_rqst::teardown(tracker_tcp_rqst* a_rqst, nconn& a_nconn,
                                   http_status_t a_status) {
  if (a_rqst && a_rqst->m_nconn) {
    int32_t l_s;
...
int32_t tracker_tcp_rqst::evr_event_timeout_cb(void* a_data) {
  ...
  // -------------------------------------------------
  nconn* l_nconn = static_cast<nconn*>(a_data);
  tracker_tcp_rqst* l_rqst =
      static_cast<tracker_tcp_rqst*>(l_nconn->get_data());
  if (l_rqst && l_rqst->m_evr_timeout) {
    l_rqst->m_evr_timeout = nullptr;
  }
...
int32_t tracker_tcp_rqst::evr_event_readable_cb(void* a_data) {
  ...
  tracker_tcp_rqst* l_rqst =
      static_cast<tracker_tcp_rqst*>(l_nconn->get_data());
  if (l_rqst && l_rqst->m_evr_readable) {
...
int32_t tracker_tcp_rqst::evr_event_writeable_cb(void* a_data) {
  ...
  tracker_tcp_rqst* l_rqst =
      static_cast<tracker_tcp_rqst*>(l_nconn->get_data());
  if (l_rqst && l_rqst->m_evr_writeable) {
...
int32_t tracker_tcp::announce(void) {
  ...
  encode_digest(l_info_hash_encoded, m_session.get_info_hash(),
                NTRNT_SHA1_SIZE);
  // -------------------------------------------------
  // port str
  // -------------------------------------------------
...
int32_t tracker_tcp::rqst_dequeue(void* a_data) {
  ...
  while (l_list.size() && !l_session.get_stopped()) {
    // -----------------------------------------
    // dequeue
    // -----------------------------------------
    if (!l_list.front()) {
...
int32_t tracker_tcp::handle_announce_response(http_resp& a_resp) {
  ...
  for (auto&& i_m : l_be.m_dict) {
    ...
    // -----------------------------------------
    if ((i_m.first == "peers") && (i_obj.m_type == BE_OBJ_STRING)) {
      int32_t l_s;
      l_s = m_session.add_peer_raw(AF_INET, (const uint8_t*)i_obj.m_ptr,
                                   i_obj.m_len, nullptr, 0,
                                   NTRNT_PEER_FROM_TRACKER);
      UNUSED(l_s);
    } else if ((i_m.first == "peers6") && (i_obj.m_type == BE_OBJ_STRING)) {
      int32_t l_s;
      l_s = m_session.add_peer_raw(AF_INET6, (const uint8_t*)i_obj.m_ptr,
                                   i_obj.m_len, nullptr, 0,
                                   NTRNT_PEER_FROM_TRACKER);
      UNUSED(l_s);
    }
...
int32_t tracker_tcp::handle_scrape_response(http_resp& a_resp) {
  ...
  for (auto&& i_m : l_be.m_dict) {
    ...
    // All in network (big endian) notation.
    // -----------------------------------------
    if ((i_m.first == "files") && (i_obj.m_type == BE_OBJ_DICT)) {
