...
int32_t upnp::init(void) {
  ...
  l_dev = upnpDiscover(2000,  // delay ms
                       nullptr, nullptr, 0, 0, 2, &l_err);
#else
  l_dev = upnpDiscover(2000,  // delay ms
                       nullptr, nullptr, 0, 0, &l_err);
#endif
#else
  l_dev = upnpDiscover(2000,  // delay ms
                       nullptr, nullptr, 0);
  ...
  if (l_dev == nullptr) {
    TRC_ERROR("performing upnpDiscover. Reason[%d]: %s", errno,
              strerror(errno));
  ...
  l_s = UPNP_GetValidIGD(l_dev, &m_urls, &m_datas, m_lan_addr,
                         sizeof(m_lan_addr));
  if (l_s != UPNP_IGD_VALID_CONNECTED) {
    TRC_ERROR("performing UPNP_GetValidIGD. Reason[%d]: %s", errno,
              strerror(errno));
    TRC_ERROR(
  ...
  // status info
  // -------------------------------------------------
  l_s = UPNP_GetStatusInfo(m_urls.controlURL, m_datas.first.servicetype,
                           m_stat_status, &m_stat_uptime, m_stat_last_conn_err);
  ...
  l_s = UPNP_GetConnectionTypeInfo(m_urls.controlURL, m_datas.first.servicetype,
  ...
  // external IP address
  // -------------------------------------------------
  l_s = UPNP_GetExternalIPAddress(m_urls.controlURL, m_datas.first.servicetype,
                                  m_stat_ext_ip);
  if (l_s != UPNPCOMMAND_SUCCESS) {
  ...
  // -------------------------------------------------
  l_s =
      UPNP_GetLinkLayerMaxBitRates(m_urls.controlURL, m_datas.first.servicetype,
                                   &m_stat_br_down, &m_stat_br_up);
  if (l_s != UPNPCOMMAND_SUCCESS) {
...
int32_t upnp::add_port_mapping(uint16_t a_port) {
  ...
  l_s = UPNP_AddPortMapping(m_urls.controlURL, m_datas.first.servicetype,
                            l_port_str, l_port_str, m_lan_addr, l_desc, "TCP",
                            nullptr, nullptr);
  if (l_s != 0) {
    TRC_ERROR("performing UPNP_AddPortMapping (TCP). Reason[%d]: %s", errno,
              strerror(errno));
  }
  ...
  l_s = UPNP_AddPortMapping(m_urls.controlURL, m_datas.first.servicetype,
                            l_port_str, l_port_str, m_lan_addr, l_desc, "UDP",
                            nullptr, nullptr);
  if (l_s != 0) {
    TRC_ERROR("performing UPNP_AddPortMapping (UDP). Reason[%d]: %s", errno,
              strerror(errno));
  }
...
int32_t upnp::delete_port_mapping(uint16_t a_port) {
  ...
  errno = 0;
  l_s = UPNP_DeletePortMapping(m_urls.controlURL, m_datas.first.servicetype,
                               l_port_str, "TCP", nullptr);
  if (l_s != 0) {
    TRC_ERROR("performing UPNP_DeletePortMapping (TCP). Reason[%d]: %s", errno,
              strerror(errno));
  }
  ...
  errno = 0;
  l_s = UPNP_DeletePortMapping(m_urls.controlURL, m_datas.first.servicetype,
                               l_port_str, "UDP", nullptr);
  if (l_s != 0) {
    TRC_ERROR("performing UPNP_DeletePortMapping (UDP). Reason[%d]: %s", errno,
              strerror(errno));
  }
...
int main(int argc, char** argv) {
  ...
  std::string l_arg;
  int l_opt_index = 0;
  struct option l_long_options[] = {{"help", no_argument, 0, 'h'},
  ...
  while ((l_opt = getopt_long_only(argc, argv, l_short_arg_list, l_long_options,
                                   &l_opt_index)) != -1) {
    if (optarg) {
