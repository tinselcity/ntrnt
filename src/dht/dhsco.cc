...
static const uint8_t s_v4_prefix[16] = {0, 0, 0,    0,    0, 0, 0, 0,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static void _make_tid(uint8_t* a_tid_return, const char* a_prefix,
                      uint16_t a_seq_no) {
...
static bool _is_local(const struct sockaddr* a_sa) {
  switch (a_sa->sa_family) {
    case AF_INET: {
      ...
      return (l_sin->sin_port == 0) || (l_addr[0] == 0) || (l_addr[0] == 127) ||
             ((l_addr[0] & 0xE0) == 0xE0);
    }
    case AF_INET6: {
      struct sockaddr_in6* l_sin6 = (struct sockaddr_in6*)a_sa;
      const uint8_t* l_addr = (const uint8_t*)&l_sin6->sin6_addr;
      return (l_sin6->sin6_port == 0) || (l_addr[0] == 0xFF) ||
             (l_addr[0] == 0xFE && (l_addr[1] & 0xC0) == 0x80) ||
             ((memcmp(l_addr, s_zeroes, 15) == 0) &&
              ((l_addr[15] == 0) || (l_addr[15] == 1))) ||
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int _parse_message(const uint8_t* a_buf, int a_buf_len,
...
#define _CHECK(ptr, len)                               \
  if (((uint8_t*)ptr) + (len) > (a_buf) + (a_buf_len)) \
    goto overflow;
  // -------------------------------------------------
  // TODO
  // -------------------------------------------------
  ...
  if (p) {
    int i = p - a_buf + 7;
    ao_msg->m_want = 0;
    while (a_buf[i] > '0' && a_buf[i] <= '9' && a_buf[i + 1] == ':' &&
           i + 2 + a_buf[i] - '0' < a_buf_len) {
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int xorcmp(const uint8_t* a_id1, const uint8_t* a_id2,
                  const uint8_t* a_ref) {
  int i_c;
...
static int _in_bucket(const uint8_t* a_id, bucket_t* a_b) {
  return (_id_cmp(a_b->m_first, a_id) <= 0) &&
         ((a_b->m_next == NULL) || (_id_cmp(a_id, a_b->m_next->m_first) < 0));
}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static bool _tid_match(const uint8_t* a_tid, const char* a_prefix,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int _insert_closest_node(uint8_t* nodes, int a_num_nodes,
                                const uint8_t* id, node_t* n) {
  int l_size;
  ...
  if (i_n < a_num_nodes - 1) {
    memmove(nodes + l_size * (i_n + 1), nodes + l_size * i_n,
            l_size * (a_num_nodes - i_n - 1));
  }
  if (n->m_ss.ss_family == AF_INET) {
...
bool _node_good(node_t* a_n, struct timeval& a_now) {
  bool l_s = false;
  l_s = ((a_n->m_pinged <= 2) && (a_n->m_reply_time >= a_now.tv_sec - 7200) &&
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int _buffer_closest_nodes(uint8_t* a_nodes, int a_num_nodes,
                                 const uint8_t* a_id, bucket_t* a_b,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static search_t* _new_search(search_t** ao_searches, uint32_t& ao_num_searches,
  ...
  while (l_sr) {
    if (l_sr->m_done &&
        (l_sr_oldest == NULL || l_sr_oldest->m_step_time > l_sr->m_step_time)) {
      l_sr_oldest = l_sr;
    }
    l_sr = l_sr->m_next;
  }
  ...
  // -------------------------------------------------
  if (l_sr_oldest && l_sr_oldest->m_done) {
    return l_sr_oldest;
  }
  // -------------------------------------------------
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static void _make_token(const struct sockaddr* a_sa, const uint8_t* a_secret,
  ...
  g_dhsco_hash_cb(ao_token_return, _DHSCO_TOKEN_SIZE, a_secret,
                  sizeof(a_secret), l_ip, l_iplen, (uint8_t*)&l_port, 2);
}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
//! \param:   TODO
//! ----------------------------------------------------------------------------
static int _token_match(const uint8_t* token, int token_len,
                        const struct sockaddr* a_sa, const uint8_t* a_secret) {
...
static search_t* _find_search(uint16_t a_tid, int a_af, search_t* a_searches) {
  ...
  while (l_sr) {
    if ((l_sr->m_tid == a_tid) && (l_sr->m_af == a_af)) {
      return l_sr;
    }
    l_sr = l_sr->m_next;
  }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_closest_nodes(const struct sockaddr* a_sa, int a_sa_len,
                              const uint8_t* a_tid, int a_tid_len,
                              const uint8_t* a_id, int a_want, int a_af,
                              storage_t* a_st, const uint8_t* a_token,
                              int a_token_len) {
  ...
  if ((a_want & WANT4)) {
    ...
    if (i_b) {
      l_num_nodes =
          _buffer_closest_nodes(l_nodes, l_num_nodes, a_id, i_b, m_now);
      if (i_b->m_next) {
        l_num_nodes = _buffer_closest_nodes(l_nodes, l_num_nodes, a_id,
                                            i_b->m_next, m_now);
      }
  ...
  if ((a_want & WANT6)) {
    ...
    if (i_b) {
      l_num_nodes6 =
          _buffer_closest_nodes(l_nodes6, l_num_nodes6, a_id, i_b, m_now);
      if (i_b->m_next) {
        l_num_nodes6 = _buffer_closest_nodes(l_nodes6, l_num_nodes6, a_id,
                                             i_b->m_next, m_now);
      }
  ...
  return send_nodes_peers(a_sa, a_sa_len, a_tid, a_tid_len, l_nodes,
                          l_num_nodes * 26, l_nodes6, l_num_nodes6 * 38, a_af,
                          a_st, a_token, a_token_len);
}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
...
search_node_t* dhsco::insert_search_node(const uint8_t* a_id,
                                         ...
                                         int a_sa_len, search_t* a_sr,
                                         int a_replied, uint8_t* a_token,
                                         int a_token_len) {
  search_node_t* i_sn;
  int i;
  int j;
...
void dhsco::insert_search_bucket(bucket_t* a_b, search_t* a_sr) {
  ...
  i_n = a_b->m_nodes;
  while (i_n) {
    insert_search_node(i_n->m_id, (struct sockaddr*)&i_n->m_ss, i_n->m_sslen,
                       a_sr, 0, NULL, 0);
    i_n = i_n->m_next;
  }
}
...
void dhsco::expire_buckets(bucket_t* a_bucket) {
  ...
  while (i_b) {
    ...
    while (i_b->m_nodes && i_b->m_nodes->m_pinged >= 4) {
      i_n = i_b->m_nodes;
    ...
    i_p = i_b->m_nodes;
    while (i_p) {
      while (i_p->m_next && i_p->m_next->m_pinged >= 4) {
        i_n = i_p->m_next;
...
void dhsco::expire_searches(dht_callback_t* a_cb, void* a_ctx) {
  ...
  while (l_sr) {
    ...
    if (l_sr->m_step_time < m_now.tv_sec - _DHSCO_SEARCH_EXPIRE_TIME) {
      ...
      if (!l_sr->m_done && a_cb) {
        (*a_cb)(a_ctx,
                l_sr->m_af == AF_INET ? DHT_EVENT_SEARCH_DONE
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::store(const uint8_t* a_id, const struct sockaddr* a_sa,
                 uint16_t a_port) {
  // -------------------------------------------------
  // get ip and len
  // -------------------------------------------------
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
void dhsco::blacklist_node(const uint8_t* a_id, const struct sockaddr* a_sa,
                           int a_sa_len) {
  node_t* l_n = nullptr;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_buf(const void* a_buf, size_t a_len, int a_flags,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_ping(const struct sockaddr* a_sa, int a_sa_len,
                     const uint8_t* a_tid, int a_tid_len) {
  char l_buf[512];
  int i_off = 0;
  int l_s;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_pong(const struct sockaddr* a_sa, int a_sa_len,
                     const uint8_t* a_tid, int a_tid_len) {
  char l_buf[512];
  int i = 0, l_s;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_find_node(const struct sockaddr* a_sa, int a_sa_len,
                          const uint8_t* a_tid, int a_tid_len,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_nodes_peers(const struct sockaddr* a_sa, int a_sa_len,
                            const uint8_t* a_tid, int a_tid_len,
                            const uint8_t* a_nodes, int a_nodes_len,
                            const uint8_t* a_nodes6, int a_nodes6_len, int a_af,
                            storage_t* a_st, const uint8_t* a_token,
                            int a_token_len) {
  char l_buf[2048];
  int i = 0;
...
int dhsco::send_cached_ping(bucket_t* a_bucket) {
  ...
  l_s = send_ping((struct sockaddr*)&a_bucket->m_cached, a_bucket->m_cached_len,
                  l_tid, 4);
...
int dhsco::upkeep_buckets(int a_family) {
  ...
  while (i_b) {
    ...
    // -----------------------------------------
    if (i_q->m_next && (i_q->m_count == 0 || (random() & 7) == 0)) {
      i_q = i_b->m_next;
    }
    if (i_q->m_count == 0 || (random() & 7) == 0) {
      bucket_t* r;
    ...
    // send find node
    // -----------------------------------------
    send_find_node((struct sockaddr*)&i_n->m_ss, i_n->m_sslen, l_tid, 4, l_id,
                   l_want, i_n->m_reply_time >= m_now.tv_sec - 15);
    // -----------------------------------------
    // pinged
    // -----------------------------------------
    pinged(i_n, i_q);
...
int dhsco::upkeep_neighborhood(int a_family) {
  ...
  if (l_n) {
    ...
    //          a_family == AF_INET6 ? " IPv6" : "");
    _make_tid(l_tid, "fn", 0);
    send_find_node((struct sockaddr*)&(l_n->m_ss), l_n->m_sslen, l_tid, 4, l_id,
                   l_want, l_n->m_reply_time >= m_now.tv_sec - 15);
    pinged(l_n, i_q);
  }
  return 1;
}
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_get_peers(const struct sockaddr* a_sa, int a_sa_len,
                          uint8_t* a_tid, int a_tid_len, uint8_t* a_infohash,
                          int a_want, int a_confirm) {
  char l_buf[512];
  int i = 0, l_s;
  l_s = snprintf(l_buf + i, 512 - i, "d1:ad2:id20:");
  INC(i, l_s, 512);
...
int dhsco::search_send_get_peers(search_t* a_sr, search_node_t* a_n) {
  ...
  if (a_n == NULL) {
    int i;
    for (i = 0; i < a_sr->m_num_nodes; ++i) {
      if (a_sr->m_nodes[i].m_pinged < 3 && !a_sr->m_nodes[i].m_replied &&
          a_sr->m_nodes[i].m_request_time <
              m_now.tv_sec - _DHSCO_SEARCH_RETRANSMIT) {
        a_n = &a_sr->m_nodes[i];
      }
    }
  }
  if (!a_n || a_n->m_pinged >= 3 || a_n->m_replied ||
      a_n->m_request_time >= m_now.tv_sec - _DHSCO_SEARCH_RETRANSMIT) {
    return 0;
  }
  //TRC_DEBUG("Sending get_peers.");
  _make_tid(l_tid, "gp", a_sr->m_tid);
  send_get_peers((struct sockaddr*)&a_n->m_ss, a_n->m_sslen, l_tid, 4,
                 a_sr->m_id, -1,
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
void dhsco::add_search_node(const uint8_t* a_id, const struct sockaddr* a_sa,
                            int a_sa_len) {
  search_t* l_sr;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
node_t* dhsco::new_node(const uint8_t* a_id, const struct sockaddr* a_sa,
...
again:
  ...
  if (_is_local(a_sa) || node_blacklisted(a_sa, a_sa_len)) {
    return NULL;
  }
  int l_my_bucket;
  ...
  while (i_n) {
    if (_id_cmp(i_n->m_id, a_id) == 0) {
      if (a_confirm || i_n->m_time < m_now.tv_sec - 15 * 60) {
  ...
  i_n = i_b->m_nodes;
  while (i_n) {
    if (i_n->m_pinged >= 3 && i_n->m_pinged_time < m_now.tv_sec - 15) {
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_announce_peer(const struct sockaddr* a_sa, int a_sa_len,
                              uint8_t* a_tid, int a_tid_len,
                              uint8_t* a_infohash, uint16_t a_port,
                              uint8_t* a_token, int a_token_len,
                              int a_confirm) {
  char l_buf[512];
  int i = 0, l_s;
  l_s = snprintf(l_buf + i, 512 - i, "d1:ad2:id20:");
  ...
  INC(i, l_s, 512);
  COPY(l_buf, i, a_infohash, 20, 512);
  l_s = snprintf(l_buf + i, 512 - i, "4:porti%ue5:token%d:", (unsigned)a_port,
                 a_token_len);
  INC(i, l_s, 512);
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_peer_announced(const struct sockaddr* a_sa, int a_sa_len,
                               uint8_t* a_tid, int a_tid_len) {
  char l_buf[512];
  int i = 0, l_s;
  l_s = snprintf(l_buf + i, 512 - i, "d1:rd2:id20:");
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::send_error(const struct sockaddr* a_sa, int a_sa_len, uint8_t* a_tid,
    ...
    if (a_sr->m_port == 0) {
      ...
      for (i = 0; i < a_sr->m_num_nodes && j < 8; ++i) {
        ...
        if (!i_sn->m_acked) {
          ...
          send_announce_peer(
              (struct sockaddr*)&i_sn->m_ss, sizeof(struct sockaddr_storage),
              l_tid, 4, a_sr->m_id, a_sr->m_port, i_sn->m_token,
              i_sn->m_token_len, i_sn->m_reply_time >= m_now.tv_sec - 15);
          i_sn->m_pinged++;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
dhsco::dhsco(int a_udp_fd, int a_udp6_fd, const uint8_t* a_id,
             const uint8_t* a_v)
    : m_myid(),
      m_have_v(0),
      m_my_v(),
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t dhsco::periodic(const void* a_buf, size_t a_buf_len,
  ...
  if ((l_msg_type < 0) || (l_msg_type == MSG_TYPE_ERROR) ||
  ...
  // rate limit?
  // -------------------------------------------------
  if ((l_msg_type > MSG_TYPE_REPLY) && rate_limit()) {
  ...
  switch (l_msg_type) {
    ...
    case MSG_TYPE_REPLY: {
      ...
      if (l_msg.m_tid_len != 4) {
        TRC_WARN("Broken node truncates transaction ids: %.*s", (int)a_buf_len,
                 (char*)a_buf);
        // ---------------------------------
      ...
      // -----------------------------------------
        ...
        //          l_gp ? " for get_peers" : "");
        if (l_msg.m_nodes_len % 26 != 0 || l_msg.m_nodes6_len % 38 != 0) {
          TRC_WARN("Unexpected length for node info!");
          blacklist_node(l_msg.m_id, a_from, a_from_len);
        } else if (l_gp && (l_sr == NULL)) {
          ...
          for (int i = 0; i < l_msg.m_nodes_len / 26; ++i) {
            ...
            if (l_sr && (l_sr->m_af == AF_INET)) {
              insert_search_node(ni, (struct sockaddr*)&sin, sizeof(sin), l_sr,
                                 0, NULL, 0);
            }
          }
          // -------------------------
        ...
        if (l_sr) {
          ...
          l_sn = insert_search_node(l_msg.m_id, a_from, a_from_len, l_sr, 1,
                                    l_msg.m_token, l_msg.m_token_len);
          UNUSED(l_sn);
          if ((l_msg.m_values_len > 0) || (l_msg.m_values6_len > 0)) {
            //TRC_DEBUG("Got values (%d+%d)!",
            //          l_msg.m_values_len / 6,
            //          l_msg.m_values6_len / 18);
            if (a_cb) {
              if (l_msg.m_values_len > 0) {
                (*a_cb)(a_ctx, DHT_EVENT_VALUES, l_sr->m_id,
                        (void*)l_msg.m_values, l_msg.m_values_len);
              }
              if (l_msg.m_values6_len > 0) {
                (*a_cb)(a_ctx, DHT_EVENT_VALUES6, l_sr->m_id,
                        (void*)l_msg.m_values6, l_msg.m_values6_len);
              }
            }
          }
        }
    ...
    case MSG_TYPE_FIND_NODE: {
      ...
      send_closest_nodes(a_from, a_from_len, l_msg.m_tid, l_msg.m_tid_len,
                         l_msg.m_target, l_msg.m_want, 0, NULL, NULL, 0);
      break;
    }
    // -------------------------------------------------
    // MSG_TYPE_GET_PEERS
    ...
    case MSG_TYPE_GET_PEERS: {
      ...
      if (_id_cmp(l_msg.m_info_hash, s_zeroes) == 0) {
        ...
        send_error(a_from, a_from_len, l_msg.m_tid, l_msg.m_tid_len, 203,
      ...
      int l_family = 0;
      if (l_st && l_st->m_num_peers > 0) {
        //TRC_DEBUG("Sending found%s peers.", a_from->sa_family == AF_INET6 ? " IPv6" : "");
        l_storage = l_st;
        l_family = a_from->sa_family;
      }
      send_closest_nodes(a_from, a_from_len, l_msg.m_tid, l_msg.m_tid_len,
                         l_msg.m_info_hash, l_msg.m_want, l_family, l_storage,
                         l_token, _DHSCO_TOKEN_SIZE);
      break;
    }
    // -------------------------------------------------
    ...
    case MSG_TYPE_ANNOUNCE_PEER: {
      ...
      if (l_msg.m_port == 0) {
        ...
        int l_s;
        l_s = send_error(a_from, a_from_len, l_msg.m_tid, l_msg.m_tid_len, 203,
      ...
      l_s =
          send_peer_announced(a_from, a_from_len, l_msg.m_tid, l_msg.m_tid_len);
      UNUSED(l_s);
      break;
    }
  }
dont_read:
  ...
  // search step
  // -------------------------------------------------
  if ((m_search_time > 0) && (m_now.tv_sec >= m_search_time)) {
    search_t* l_sr;
    l_sr = m_searches;
    while (l_sr) {
      if (!l_sr->m_done &&
          l_sr->m_step_time + _DHSCO_SEARCH_RETRANSMIT / 2 + 1 <=
              m_now.tv_sec) {
    ...
    while (l_sr) {
      if (!l_sr->m_done) {
        time_t l_tm;
        l_tm = l_sr->m_step_time + _DHSCO_SEARCH_RETRANSMIT +
               random() % _DHSCO_SEARCH_RETRANSMIT;
        if (m_search_time == 0 || m_search_time > l_tm) {
          m_search_time = l_tm;
        }
      }
      l_sr = l_sr->m_next;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::search(const uint8_t* a_id, int a_port, int a_af,
                  dht_callback_t* a_cb, void* a_ctx) {
  bucket_t* l_buckets = m_buckets;
  int l_sr_duplicate;
  if (a_af != AF_INET) {
    l_buckets = m_buckets6;
  ...
  search_t* l_sr = m_searches;
  while (l_sr) {
    if ((l_sr->m_af == a_af) && (_id_cmp(l_sr->m_id, a_id) == 0)) {
      break;
    }
    l_sr = l_sr->m_next;
  }
  ...
  again:
    for (i = 0; i < l_sr->m_num_nodes; ++i) {
      ...
      if ((n->m_pinged >= 3) || (n->m_reply_time < m_now.tv_sec - 7200)) {
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int dhsco::status(int a_family, int* a_good, int* a_dubious, int* a_cached,
                  int* a_incoming) {
  int l_good = 0;
  int l_dubious = 0;
  int l_cached = 0;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static void _display_buckets(bucket_t& a_b, const uint8_t* a_myid,
                             struct timeval& a_now) {
  node_t* i_n = a_b.m_nodes;
  NDBG_OUTPUT("Bucket ");
  // shorten display output 20 -> 4
  _display_hex(a_b.m_first, 20);
  NDBG_OUTPUT(" count %d/%d age %d%s%s:\n", a_b.m_count, a_b.m_max_count,
  ...
  while (i_n) {
    ...
    if (i_n->m_ss.ss_family == AF_INET6) {
      NDBG_OUTPUT(" [%s%40s%s]:%s%6u%s ", ANSI_COLOR_FG_YELLOW, l_buf,
                  ANSI_COLOR_OFF, ANSI_COLOR_FG_CYAN, l_port, ANSI_COLOR_OFF);
    } else {
      NDBG_OUTPUT(" %s%42s%s:%s%6u%s ", ANSI_COLOR_FG_YELLOW, l_buf,
                  ANSI_COLOR_OFF, ANSI_COLOR_FG_CYAN, l_port, ANSI_COLOR_OFF);
    }
    if (i_n->m_time != i_n->m_reply_time) {
      NDBG_OUTPUT("age %12ld, %12ld", (long)(a_now.tv_sec - i_n->m_time),
                  (long)(a_now.tv_sec - i_n->m_reply_time));
    } else {
      NDBG_OUTPUT("age %12ld, ____________",
...
void dhsco::display(void) {
  ...
  while (i_st) {
    ...
    for (int i = 0; i < i_st->m_num_peers; ++i) {
      ...
      NDBG_OUTPUT(" %40s:%u (%ld)", l_buf, i_st->m_peers[i].m_port,
...
#define IF_MEM(_m, _str)                        \
  else if ((strncmp(_m->name.GetString(), _str, \
                    _m->name.GetStringLength())) == 0)
  CHECK_OBJ(l_doc);
  // -------------------------------------------------
  // Iterate over objects...
  // -------------------------------------------------
  for (rapidjson::Value::ConstMemberIterator i_m = l_doc.MemberBegin();
       i_m != l_doc.MemberEnd(); ++i_m) {
    if (0) {}
    // -----------------------------------------
    // id
    // -----------------------------------------
