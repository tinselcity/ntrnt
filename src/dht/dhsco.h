...
//! callbacks
//! ----------------------------------------------------------------------------
typedef int32_t (*dhsco_sendto_cb_t)(int, const void*, int, int, const struct sockaddr*, int);
typedef bool (*dhsco_blacklisted_cb_t)(const struct sockaddr*, int);
typedef void (*dhsco_hash_cb_t)(void*, int, const void*, int, const void*, int, const void*, int);
typedef int (*dhsco_random_bytes_cb_t)(void*, size_t);
typedef void dht_callback_t(void*, dht_event_t, const uint8_t*, const void*, size_t);
//! ----------------------------------------------------------------------------
//! user defined
//! ----------------------------------------------------------------------------
...
public:
        ...
        // running
        // -------------------------------------------------
        int32_t periodic(const void* a_buf,
                         size_t a_buf_len,
        ...
        // search dht by id
        // -------------------------------------------------
        int search(const uint8_t *a_id, int a_port, int a_af, dht_callback_t *a_cb, void* a_ctx);
        // -------------------------------------------------
        // get current health of the dht
        // -------------------------------------------------
        int status(int a_family, int* a_good, int* a_dubious, int* a_cached, int* a_incoming);
...
private:
        ...
        int send_buf(const void* a_buf,
                     size_t a_len,
                     int a_flags,
                     const struct sockaddr* a_sa,
                     int a_sa_len);
        int send_ping(const struct sockaddr* a_sa,
                      int a_sa_len,
                      const uint8_t* a_tid,
                      int a_tid_len);
        int send_pong(const struct sockaddr* a_sa,
                      int a_sa_len,
                      const uint8_t* a_tid,
                      int a_tid_len);
        int send_find_node(const struct sockaddr* a_sa,
                           int a_sa_len,
                           const uint8_t* a_tid,
                           int a_tid_len,
                           ...
                           int a_want,
                           int a_confirm);
        int send_nodes_peers(const struct sockaddr* a_sa,
                             int a_sa_len,
                             const uint8_t* a_tid,
                             int a_tid_len,
                             const uint8_t* nodes,
                             ...
                             const uint8_t* token,
                             int token_len);
        int send_announce_peer(const struct sockaddr* a_sa,
                               int a_sa_len,
                               uint8_t* a_tid,
                               int a_tid_len,
                               uint8_t* a_infohash,
                               ...
                               int a_token_len,
                               int a_confirm);
        int send_peer_announced(const struct sockaddr* a_sa,
                                int a_sa_len,
                                uint8_t* a_tid,
                                int a_tid_len);
        int send_error(const struct sockaddr* a_sa,
                       int a_sa_len,
                       uint8_t* a_tid,
                       int a_tid_len,
                       int a_code,
                       const char *a_msg);
        int send_get_peers(const struct sockaddr* a_sa,
                           int a_sa_len,
                           uint8_t* a_tid,
                           int a_tid_len,
                           uint8_t* a_infohash,
        ...
        void add_search_node(const uint8_t* a_id,
                             const struct sockaddr* a_sa,
                             int a_sa_len);
        node_t* new_node(const uint8_t* a_id,
                         const struct sockaddr* a_sa,
                         int a_sa_len,
                         int a_confirm);
        int split_bucket(bucket_t* a_b);
        int split_bucket_helper(bucket_t* a_b, node_t** ao_nodes_return);
        int send_closest_nodes(const struct sockaddr* a_sa,
                               int a_sa_len,
                               const uint8_t* a_tid,
                               int a_tid_len,
                               const uint8_t* id,
                               ...
                               const uint8_t* token,
                               int token_len);
        int store(const uint8_t* id,
                  const struct sockaddr* a_sa,
                  uint16_t port);
        void blacklist_node(const uint8_t*a_id,
                            const struct sockaddr* a_sa,
                            int a_sa_len);
        // -------------------------------------------------
