...
int32_t read_file(const char* a_file, char** a_buf, size_t* a_len) {
  ...
  if (l_status != 0) {
    NTRNT_PERROR("error performing stat on file: %s.  Reason: %s", a_file,
                 strerror(errno));
    return NTRNT_STATUS_ERROR;
  }
  if (!(l_stat.st_mode & S_IFREG)) {
    NTRNT_PERROR("error opening file: %s.  Reason: is NOT a regular file",
                 a_file);
    return NTRNT_STATUS_ERROR;
  }
  FILE* l_file;
  ...
  if (NULL == l_file) {
    NTRNT_PERROR("error opening file: %s.  Reason: %s", a_file,
                 strerror(errno));
    return NTRNT_STATUS_ERROR;
  }
  ...
  if (l_read_size != l_size) {
    NTRNT_PERROR("error performing fread.  Reason: %s [%d:%d]", strerror(errno),
                 l_read_size, l_size);
    return NTRNT_STATUS_ERROR;
  }
  l_buf[l_size] = '\0';
...
int32_t ensure_dir(const std::string& a_dir) {
  ...
  if (errno != ENOENT) {
    TRC_ERROR("performing stat [DIR: %s].  Reason[%d]: %s", a_dir.c_str(),
  ...
  if (l_s != 0) {
    TRC_ERROR("performing mkdir [DIR: %s].  Reason[%d]: %s", a_dir.c_str(),
              errno, strerror(errno));
    return NTRNT_STATUS_ERROR;
  }
  return NTRNT_STATUS_OK;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t b64_encode(std::string& ao_out, const unsigned char* a_in,
                   size_t a_in_len) {
  char* l_buf = nullptr;
  int32_t l_s;
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t hex2bin(uint8_t* ao_bin, size_t& ao_bin_len, const char* a_hex,
                const size_t a_hex_len) {
  if ((a_hex == NULL) || (a_hex_len == 0)) {
    return NTRNT_STATUS_ERROR;
  }
  // -------------------------------------------------
  // set
