...
//! \param   TODO
//! ----------------------------------------------------------------------------
geoip2_mmdb::geoip2_mmdb() : m_init(false), m_city_mmdb(NULL) {}
//! ----------------------------------------------------------------------------
//! \details TODO
//! \return  TODO
//! \param   TODO
...
int32_t geoip2_mmdb::init(const std::string& a_city_mmdb_path) {
  ...
  if (l_s != MMDB_SUCCESS) {
    TRC_ERROR("Can't open city mmdb file %s. Reason: %s",
              a_city_mmdb_path.c_str(), MMDB_strerror(l_s));
    if (l_db) {
      free(l_db);
      l_db = NULL;
    }
...
int32_t geoip2_mmdb::get_geoip_data(const char** ao_cn_name,
                                    ...
                                    uint32_t& ao_city_name_len, double& ao_lat,
                                    double& ao_longit, const char* a_ip,
                                    uint32_t a_ip_len) {
  if (!ao_cn_name || !ao_city_name) {
  ...
  *ao_city_name = NULL;
  ...
  // *************************************************
  // -------------------------------------------------
  l_s = MMDB_get_value(&l_ls.entry, &l_e_dat, "country", "names", "en", NULL);
  if (l_s != MMDB_SUCCESS) {
...
lookup_city:
  ...
  // *************************************************
  // -------------------------------------------------
  l_s = MMDB_get_value(&l_ls.entry, &l_e_dat, "city", "names", "en", NULL);
  if (l_s != MMDB_SUCCESS) {
...
lookup_lat:
  ...
  // *************************************************
  // -------------------------------------------------
  l_s = MMDB_get_value(&l_ls.entry, &l_e_dat, "location", "latitude", NULL);
  if (l_s != MMDB_SUCCESS) {
    //TRC_ERROR("looking up the entry for ip: %.*s: reason: %s", (int)a_ip_len, a_ip, MMDB_strerror(l_s));
    goto lookup_lon;
  }
...
lookup_lon:
  ...
  // *************************************************
  // -------------------------------------------------
  l_s = MMDB_get_value(&l_ls.entry, &l_e_dat, "location", "longitude", NULL);
  if (l_s != MMDB_SUCCESS) {
    //TRC_ERROR("looking up the entry for ip: %.*s: reason: %s", (int)a_ip_len, a_ip, MMDB_strerror(l_s));
    goto done;
  }
