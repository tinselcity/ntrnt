#ifndef _NTRNT_DATA_H
#define _NTRNT_DATA_H
//! ----------------------------------------------------------------------------
//! includes
//! ----------------------------------------------------------------------------
#include <stdint.h>
...
typedef struct _data {
  const char* m_data;
  uint32_t m_len;
  _data(void) : m_data(NULL), m_len(0) {}
  void clear(void) {
    m_data = NULL;
    m_len = 0;
  }
...
typedef struct _mutable_data {
  char* m_data;
  uint32_t m_len;
  _mutable_data(void) : m_data(NULL), m_len(0) {}
  void clear(void) {
    m_data = NULL;
    m_len = 0;
  }
...
typedef std::map<mutable_data_t, mutable_data_list_t, mutable_data_case_i_comp>
    mutable_data_map_list_t;
// ---------------------------------------------------------
// find_first helper
// ---------------------------------------------------------
inline bool find_first(mutable_data_t& ao_val,
                       const mutable_data_map_list_t& a_map, const char* a_key,
                       uint32_t a_len) {
  ao_val.m_data = NULL;
  ao_val.m_len = 0;
  // TODO -why use mutable?
  mutable_data_t l_d;
  l_d.m_data = const_cast<char*>(a_key);
  l_d.m_len = a_len;
