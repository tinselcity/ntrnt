...
static struct CRYPTO_dynlock_value* dyn_create_function(const char* a_file,
                                                        int a_line) {
...
//! ----------------------------------------------------------------------------
#if OPENSSL_VERSION_NUMBER < 0x10100000L
static void dyn_lock_function(int a_mode, struct CRYPTO_dynlock_value* a_l,
...
//! ----------------------------------------------------------------------------
#if OPENSSL_VERSION_NUMBER < 0x10100000L
static void pthreads_locking_callback(int a_mode, int a_type,
                                      const char* a_file, int a_line) {
#if 0
...
static void tls_init_locking(void) {
  ...
  g_lock_cs =
      (pthread_mutex_t*)OPENSSL_malloc(l_num_locks * sizeof(pthread_mutex_t));
...
int32_t get_tls_options_str_val(const std::string a_options_str, long& ao_val) {
  ...
  // Remove whitespace
  l_options_str.erase(
      std::remove_if(l_options_str.begin(), l_options_str.end(), ::isspace),
      l_options_str.end());
  ao_val = 0;
  std::string l_token;
  std::string l_delim = "|";
...
int tls_cert_verify_callback_allow_self_signed(int ok, X509_STORE_CTX* store) {
  ...
  if ((err == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) ||
    ...
    snprintf(
        gts_last_tls_error, sizeof(gts_last_tls_error),
...
int tls_cert_verify_callback(int ok, X509_STORE_CTX* store) {
  ...
  if (store) {
    ...
    //int depth = X509_STORE_CTX_get_error_depth(store);
    int err = X509_STORE_CTX_get_error(store);
    snprintf(gts_last_tls_error, 256, "Error[%d].  Reason: %s", err,
...
bool tls_x509_get_ids(X509* x509, std::vector<std::string>& ids) {
  ...
  if (l_names) {
    ...
    for (int i = 0; i < sk_GENERAL_NAME_num(l_names); ++i) {
      ...
      if (i_name->type == GEN_DNS) {
        l_san.assign(reinterpret_cast<char*>(
                         ASN1_STRING_data(i_name->d.uniformResourceIdentifier)),
  ...
  while ((i = X509_NAME_get_index_by_NID(l_subj, NID_commonName, i)) != -1) {
    ASN1_STRING* i_name =
        X509_NAME_ENTRY_get_data(X509_NAME_get_entry(l_subj, i));
...
//! ----------------------------------------------------------------------------
static int validate_server_certificate_hostname(X509* a_cert,
                                                const char* a_host) {
...
//! \notes:   Based on example from "Network Security with OpenSSL" pg. 132
//! ----------------------------------------------------------------------------
int32_t validate_server_certificate(SSL* a_tls, const char* a_host,
                                    bool a_disallow_self_signed) {
  X509* l_cert = NULL;
  // Get certificate
  ...
  if (a_host) {
    ...
    if (0 != l_status) {
      snprintf(gts_last_tls_error, sizeof(gts_last_tls_error),
