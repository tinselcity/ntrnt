...
#define ENSURE_POD(class_name)                      \
  void __do_check_pod_##class_name() const {        \
    if (0) {                                        \
      static_assert(std::is_pod<class_name>::value, \
                    #class_name "must be POD");     \
    }                                               \
  }
#endif
...
#define DISALLOW_ASSIGN(class_name) class_name& operator=(const class_name&);
#endif
...
#define DISALLOW_DEFAULT_CTOR(class_name) class_name();
#endif
#ifndef UNUSED
