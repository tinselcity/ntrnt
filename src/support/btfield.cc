...
btfield::btfield(uint32_t a_size)
    : m_bits(a_size), m_raw(nullptr), m_raw_len(0), m_dirty(true) {}
//! ----------------------------------------------------------------------------
//! \details: TODO
//! \return:  TODO
//! \param:   TODO
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t btfield::import_raw(const uint8_t* a_buf, size_t a_len,
                            size_t a_bit_len) {
  ...
  size_t l_bt = 0;
  for (size_t i_byte = 0; (i_byte < a_len) && (l_bt < a_bit_len); ++i_byte) {
    uint8_t l_byte = a_buf[i_byte];
    for (uint8_t i_bt = _BTFIELD_BYTE_LEN; (i_bt) && (l_bt < a_bit_len);
         --i_bt, ++l_bt) {
...
int32_t btfield::export_raw(uint8_t** ao_buf, size_t& ao_len) {
  ...
  m_raw_len =
      l_bts_size / _BTFIELD_BYTE_LEN + (l_bts_size % _BTFIELD_BYTE_LEN ? 1 : 0);
  m_raw = (uint8_t*)malloc(sizeof(uint8_t) * m_raw_len);
  memset(m_raw, 0, m_raw_len);
  size_t l_bt = 0;
  for (size_t i_byte = 0; (i_byte < m_raw_len) && (l_bt < l_bts_size);
       ++i_byte) {
    for (uint8_t i_bt = _BTFIELD_BYTE_LEN; (i_bt) && (l_bt < l_bts_size);
         --i_bt, ++l_bt) {
