...
typedef struct nb_struct {
  ...
  nb_struct(size_t a_len)
      : m_data(nullptr), m_len(a_len), m_written(0), m_read(0), m_ref(false) {}
  // -------------------------------------------------
  // std constructor
  // -------------------------------------------------
  nb_struct(char* a_buf, size_t a_len)
      : m_data(a_buf), m_len(a_len), m_written(a_len), m_read(0), m_ref(true) {}
  // -------------------------------------------------
  // destructor
  // -------------------------------------------------
  ~nb_struct(void) {
...
off_t nbq::write_fd(int a_fd, size_t a_len, ssize_t& a_status) {
  ...
  while (l_left) {
    ...
    if (a_status > 0) {
      ...
      l_left -= a_status;
      l_written += a_status;
    } else if ((a_status == 0) || ((a_status < 0) && (errno == EAGAIN))) {
      break;
    } else if (a_status < 0) {
...
void nbq::reset_read(void) {
  ...
  m_total_read_avail = 0;
  for (nb_list_t::const_iterator i_b = m_q.begin(); i_b != m_q.end(); ++i_b) {
    (*i_b)->read_reset();
...
void nbq::reset_write(void) {
  for (nb_list_t::iterator i_b = m_q.begin(); i_b != m_q.end();) {
    if (!(*i_b)) {
      ++i_b;
      continue;
    }
...
void nbq::shrink(void) {
  ...
  if ((m_cur_read_block != m_q.end()) && (m_cur_write_block != m_q.end()) &&
      (*m_cur_read_block == *m_cur_write_block)) {
    nb_t* l_nb = (*m_cur_read_block);
    if ((l_nb->read_avail() == 0) && (l_nb->write_avail() == 0)) {
      delete l_nb;
      l_nb = nullptr;
      m_q.clear();
      m_cur_write_block = m_q.end();
...
int32_t nbq::split(nbq** ao_nbq_tail, size_t a_offset) {
  *ao_nbq_tail = nullptr;
  ...
  for (i_b = m_q.begin(); i_b != m_q.end(); ++i_b) {
    if (!(*i_b)) {
...
void nbq::b_write_incr(size_t a_len) {
  ...
  if ((a_len > 0) && ((*m_cur_read_block)->read_avail() == 0)) {
    ++m_cur_read_block;
  }
}
//! ----------------------------------------------------------------------------
