...
#define SET_SOCK_OPT(_sock_fd, _sock_opt_level, _sock_opt_name, _sock_opt_val) \
  do {                                                                         \
    ...
    if (_l_status == -1) {                                                     \
      TRC_ERROR("Failed to set sock_opt: %s.  Reason: %s.", #_sock_opt_name,   \
                strerror(errno));                                              \
  ...
  if (m_evr_loop) {
    if (0 != m_evr_loop->add_fd(
                 ...
                 EVR_FILE_ATTR_MASK_READ | EVR_FILE_ATTR_MASK_STATUS_ERROR |
                     ...
                     EVR_FILE_ATTR_MASK_ET,
                 &m_evr_fd)) {
      TRC_ERROR("Couldn't add socket fd[%d]", m_fd);
      return NC_STATUS_ERROR;
    }
  }
  return NC_STATUS_OK;
...
int32_t nconn_tcp::ncread(char* a_buf, uint32_t a_buf_len) {
  ...
  TRC_ALL("HOST[%s] fd[%3d] READ: %zd bytes. buf: %p. Reason: %s",
          m_label.c_str(), m_fd, l_s, a_buf, ::strerror(errno));
  if (l_s > 0)
  ...
  if (l_s > 0) {
    ...
    switch (errno) {
      ...
      case ECONNRESET: {
        TRC_WARN("recvfrom: %s", strerror(errno));
        return NC_STATUS_ERROR;
      }
      default: {
        TRC_WARN("recvfrom: %s", strerror(errno));
...
#else
  m_fd = ::socket(m_host_info.m_sock_family, m_host_info.m_sock_type,
                  m_host_info.m_sock_protocol);
  fcntl(m_fd, F_SETFD, FD_CLOEXEC);
#endif
  if (m_fd < 0) {
    NCONN_ERROR(CONN_STATUS_ERROR_INTERNAL,
                "LABEL[%s]: Error creating socket. Reason: %s", m_label.c_str(),
                ::strerror(errno));
    return NC_STATUS_ERROR;
  }
  // -------------------------------------------------
...
int32_t nconn_tcp::ncaccept() {
  ...
  // -------------------------------------------------
#ifdef __linux__
  l_fd = accept4(m_fd, (struct sockaddr*)&m_remote_sa, &m_remote_sa_len,
  ...
  if (l_fd < 0) {
    switch (errno) {
      ...
      default: {
        TRC_ERROR("accept failed. Reason[%d]: %s", errno, ::strerror(errno));
        return NC_STATUS_ERROR;
      }
    }
  }
  ...
  if (l_flags == -1) {
    TRC_ERROR("fcntl failed. Reason[%d]: %s", errno, ::strerror(errno));
    return NC_STATUS_ERROR;
  }
  errno = 0;
...
state_top:
  ...
  // try connect
  // -------------------------------------------------
  l_cs = ::connect(m_fd, ((struct sockaddr*)&(m_host_info.m_sa)),
  ...
  if (l_cs < 0) {
    switch (errno) {
      ...
      case EINVAL: {
        ...
        if (::getsockopt(m_fd, SOL_SOCKET, SO_ERROR, (void*)&l_err, &l_errlen) <
            0) {
          NCONN_ERROR(
      ...
      case ECONNREFUSED: {
        TRC_ERROR("LABEL[%s]: connect failed. Reason: %s", m_label.c_str(),
                  strerror(errno));
        return NC_STATUS_ERROR;
      }
      case EAGAIN:
      ...
      default: {
        NCONN_ERROR(CONN_STATUS_ERROR_INTERNAL,
                    "LABEL[%s]: Error Unkown. Reason: %s", m_label.c_str(),
                    ::strerror(errno));
        return NC_STATUS_ERROR;
      }
    }
