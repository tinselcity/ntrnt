...
int32_t nconn_tls::init(void)
{
        ...
        if (!m_ssl_ctx)
        {
                NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: ctx == NULL", m_label.c_str());
        ...
        if (!m_ssl)
        {
                ...
                if (!m_ssl)
                {
                        NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: tls_ctx == NULL", m_label.c_str());
                        return NC_STATUS_ERROR;
                }
        }
        // -------------------------------------------------
        ...
        if (!m_tls_opt_cipher_str.empty())
        {
                ...
                {
                        NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: Failed to set tls cipher list: %s", m_label.c_str(), m_tls_opt_cipher_str.c_str());
                        return NC_STATUS_ERROR;
                }
        }
        // -------------------------------------------------
           ...
           !m_tls_opt_hostname.empty())
        {
                ...
                {
                        NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: Failed to set tls hostname: %s", m_label.c_str(), m_tls_opt_hostname.c_str());
                        return NC_STATUS_ERROR;
                }
        }
        // -------------------------------------------------
...
int32_t nconn_tls::tls_connect(void)
{
        ...
        if (l_s <= 0)
        {
                ...
                case SSL_ERROR_SSL:
                {
                        ...
                        if (l_buf)
                        {
                                NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: TLS_ERROR[%ld]: %s.", m_label.c_str(), m_last_err, l_buf);
                        }
                        else
                        {
                                NCONN_ERROR(CONN_STATUS_ERROR_CONNECT_TLS, "LABEL[%s]: TLS_ERROR[%ld]: OTHER.",m_label.c_str(), m_last_err);
                        }
                        break;
                }
