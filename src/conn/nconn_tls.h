#ifndef _NTRNT_NCONN_TLS_H
#define _NTRNT_NCONN_TLS_H
//! ----------------------------------------------------------------------------
//! includes
//! ----------------------------------------------------------------------------
...
class nconn_tls : public nconn_tcp {
 public:
  ...
  bool is_connecting(void) {
    return ((m_tls_state == TLS_STATE_CONNECTING) ||
  ...
  bool is_accepting(void) {
    return ((m_tls_state == TLS_STATE_ACCEPTING) ||
...
//! \prototypes:
//! ----------------------------------------------------------------------------
SSL_CTX* tls_init_ctx(const std::string& a_cipher_list, long a_options = 0,
