#ifndef _NTRNT_CONN_STATUS_H
#define _NTRNT_CONN_STATUS_H
//! ----------------------------------------------------------------------------
//! includes
//! ----------------------------------------------------------------------------
#include <string>
...
typedef enum {
  ...
  CONN_STATUS_ERROR_CONNECT =
      -3,  // resolved but failed to TCP connect, explicit
  ...
  CONN_STATUS_ERROR_CONNECT_TLS_HOST =
      -6,  // TCP connected but TLS error, specific error for hostname validation failure because SSL makes us do it ourselves
  ...
  CONN_STATUS_ERROR_TIMEOUT =
      -11,  // got a timeout waiting for something, generic.  TODO: deprecate after independent connect/send/recv timeout support
