...
int32_t bdecode::init(void) {
  ...
  if (*m_cur_ptr != 'd') {
    TRC_ERROR(
        "metainfo file does not appear to bdecode a dict -no preceding 'd'");
...
int32_t bdecode::parse_int(be_int_t& ao_int) {
  ...
  // -------------------------------------------------
  const char* l_len_begin = m_cur_ptr;
  while ((*m_cur_ptr == '-') || isdigit(*m_cur_ptr)) {
    _INCR_PTR();
  }
  char* l_len_end = const_cast<char*>(m_cur_ptr);
  // -------------------------------------------------
...
bencode_writer::bencode_writer(void)
    : m_root(), m_dict(), m_cur_key(), m_cur_obj(nullptr), m_data() {
  m_root.m_len = 0;
...
void bencode_writer::w_start_list(void) {
  ...
  // -------------------------------------------------
  if ((l_cur_obj->m_type != BE_OBJ_DICT) || (m_cur_key.empty())) {
    TRC_ERROR("error");
    return;
  }
  // -------------------------------------------------
...
void bencode_writer::s_dict(const be_dict_t& a_dict) {
  ...
  for (auto&& i_m : a_dict) {
    ...
    m_data.insert(m_data.end(), l_len_str,
                  l_len_str + strnlen(l_len_str, sizeof(l_len_str)));
    m_data.push_back(':');
    m_data.insert(m_data.end(), i_m.first.c_str(),
                  i_m.first.c_str() + i_m.first.length());
    // -----------------------------------------
    // write obj
    // -----------------------------------------
    s_obj(i_m.second);
...
void bencode_writer::s_obj(const be_obj_t& a_obj) {
  switch (a_obj.m_type) {
    ...
    case BE_OBJ_INT: {
      ...
      // -----------------------------------------
      m_data.push_back('i');
      m_data.insert(m_data.end(), l_int_str,
                    l_int_str + strnlen(l_int_str, sizeof(l_int_str)));
      m_data.push_back('e');
      break;
    }
    // -------------------------------------------------
    ...
    case BE_OBJ_MUTABLE_STRING: {
      ...
      // get length string
      // -----------------------------------------
      const be_mutable_string_t& l_obj =
          *((const be_mutable_string_t*)a_obj.m_obj);
      char l_len_str[16];
      ...
      // -----------------------------------------
      m_data.insert(m_data.end(), l_len_str,
                    l_len_str + strnlen(l_len_str, 16));
      m_data.push_back(':');
      // -----------------------------------------
      // append string
      // -----------------------------------------
